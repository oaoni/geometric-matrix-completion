...
class SGMC(GMC):
    ...
    def __init__(self,M,M_training,M_test,S_training,S_test,
                 ...
                 process_side=True,W_val=None,init_vec=[['eye',1],['fmc_default',0],['eye',1]],
        ...
        with g.as_default():
            ...
            self.build_model(M_training,M_test,S_training,S_test,self.eig_vecs_row,self.eig_vecs_col,
                        self.eig_vals_row,self.eig_vals_col,p_max,q_max,p_init,q_init,m,n,
                        mu_r,mu_c,rho_r,rho_c,alpha,DMF,init_vec)
...
class symSGMC(GMC):
    ...
    def build_model(self,M_training,M_test,S_training,S_test,eig_vecs_row,eig_vecs_col,
        ...
        C_new = tf.matmul(tf.matmul(P_tf, C_tf), tf.transpose(P_tf))
        self.X = tf.matmul(tf.matmul(Phi_tf, C_new), tf.transpose(Phi_tf))
        X = self.X
        E_data = self._squared_frobenius_norm(tf.multiply(self.X, S_training_tf) - M_training)
        ...
        if DMF:
            ...
            # E_tot = E_data + mu_r*E_dirichlet_rows+mu_c*E_dirichlet_cols+rho_r*E_diag_rows+rho_c*E_diag_cols
            E_tot = E_data + mu_r*E_dirichlet_rows + rho_r*E_diag_rows
